...
impl FourValues {
    pub fn lerp(&self, value: &f64) -> f64 {
        /*
        	...
        	*/
        let numerator =
            (self.min_out * (self.max_in - value)) + (self.max_out * (value - self.min_in));
...
pub fn initcolormap() -> Vec<ReturnColor> {
    /*
    	...
    	*/
    let stops = vec![
        0.0, 100.0, 125.0, 250.0, 400.0, 500.0, 650.0, 800.0, 950.0, 1150.0, 1400.0, 1700.0,
        2000.0, 2300.0, 2600.0, 2900.0,
    ];

    let black = ReturnColor { r: 0, g: 0, b: 0 };
    ...
    let green = ReturnColor { r: 0, g: 255, b: 0 };
    let red = ReturnColor {
        r: 230,
        g: 66,
        b: 16,
...
pub fn cache_calculate(params: &Parameters, precision: u32) -> Storage {
    defined_cache_calculate(params, precision, num_cpus::get())
}

pub fn defined_cache_calculate(params: &Parameters, precision: u32, threads: usize) -> Storage {
    /*
    ...
    for x in linspace(&params.low_x, &high_x, width, precision) {
        for y in linspace(&params.low_y, &high_y, height, precision) {
            ...
            let imag_clip = to_usize(&ay.clone().mul_sub_mul(
                &params.zoom,
                &params.low_y,
                &params.zoom,
            ));
            let real_clip = to_usize(&ax.clone().mul_sub_mul(
                &params.zoom,
                &params.low_x,
                &params.zoom,
            ));
